...
export default function correctSentence(text) {
  if (!text) return text;
  if (text.charAt(0).toUpperCase != text.charAt(0)) text = text.charAt(0).toUpperCase()+text.slice(1);
  if (text.substr(text.length-1) != '.' && text.substr(text.length-1)!= '?' && text.substr(text.length-1)!='!'){
    text=text + '.';
  }; 
  return text;
}
